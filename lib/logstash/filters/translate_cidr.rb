...
# 
# We adapt this module to take a CIDR record as hash key.
#
...
class LogStash::Filters::Translate_CIDR < LogStash::Filters::Base
  config_name "translate_cidr"
  ...
  # Same as regex, but treat dictionary keys matching a CIDR record.
  # Note: this is activated only when `exact => true`.
  config :cidr, :validate => :boolean, :default => true
